...
const pages = [
  { view: "owner-mobile", file: "m-01-home.png" },
  { view: "owner-hos", file: "m-02-hos-risks.png" },
  { view: "owner-overview", file: "m-03-owner-overview.png" },
  { view: "owner-recap", file: "m-04-daily-recap.png" },
  { view: "netradyne-dashboard", file: "m-05-netradyne.png" },
  { view: "owner-issues", file: "m-06-issues.png" },
];
...
// open hamburger menu helper
async function openMenu() {
  const btn = await page.$('button:has-text("Menu"), .topbar button, [aria-label="Menu"]');
  // Try common selector for hamburger in this app
  const hamburger = await page.$('button[aria-label="Toggle menu"], .menu-btn, header button');
  if (hamburger) await hamburger.click();
}

for (const p of pages) {
  const hamburger = await page.$('[data-action="toggle-sidebar"].hamburger-btn');
  if (hamburger) { await hamburger.click(); await page.waitForTimeout(400); }
  const btn = await page.$(`[data-view="${p.view}"]`);
  ...
  await page.screenshot({ path: dest, fullPage: !!p.full });
